...
H1. Groups will outperform individuals in the training sessions across levels of complexity. H2. Groups will outperform individuals in abstraction levels H2a. Groups will outperform individuals in the transfer task across levels of complexity. H2b. Groups will outperform individuals in the first cycle of each session (before any tokens get repeated). H3. Variability in group performance will be contingent on semantic distance
...
We use a mixed logistic regression model to assess main effects of condition (pair vs. individual) and complexity (session 1 to 3). We account for individual variability (random intercept plus random slopes due to session) and stimulus variability (random intercepts). Hypothesis testing is on the %
...
d2$Preds <- predict(H2a_m)[,1]
dd <- d2 %>%
  group_by(subject, condition, session) %>%
  summarize(Accuracy1 = mean(Preds, na.rm=T), Accuracy2 = mean(Accuracy,na.rm=T))
write_csv(dd, here("data","Fig4_data.csv"))
...
Fig4 <- ggplot(dd) + 
  ...
  scale_color_brewer(palette="Dark2") +
  theme_classic() +
  labs(x = "Complexity level", y = "Transfer phase performance (accuracy)", color = "Condition")

ggsave(here("plots","figure4.pdf"), Fig4, width = 5, height = 4)
ggsave(here("plots","Figure4.svg"), Fig4, height = 4, width = 5)
...
write_csv(d1a, here("data","Fig3_data.csv"))
# Figure 3
...
Fig3 <- ggplot(d1a) +
  geom_smooth(aes(trial,Accuracy, color = condition), 
              method = "scam", 
              formula = y ~ s(x, k = 9, bs = "mpi"), 
              se = T) +
...
write_csv(FinalData, here("data", "Fig5_data.csv"))
...
## Testing whether just dyads could be better simply because one fo the two individuals might be better than the other.
```{r}

d <- d %>% mutate(correct = as.numeric(correct)-1)

result <- d %>% subset(test==1) %>% group_by(subject, condition, session) %>% summarize(accuracy = mean(correct))
result_ind <- result %>% subset(condition=="Individuals")
result_dya <- result %>% subset(condition=="Dyads")

fake_dyads <- combn(unique(result_ind$subject), 2)

result_fake <- tibble(subject=NULL, condition=NULL, session = NULL, accuracy = NULL)

for (fako in seq(ncol(fake_dyads))){
  for (comp in c(1:3)){
    
    accuracy <- max(result_ind$accuracy[result_ind$subject==fake_dyads[1,fako] & result_ind$session==comp], result_ind$accuracy[result_ind$subject==fake_dyads[2,fako] & result_ind$session==comp])
    
    temp <- tibble(subject=fako, condition="fakeDyads", session = comp, accuracy = accuracy)
    result_fake <- rbind(result_fake, temp)
...
result_fake$subject<- as.factor(result_fake$subject)

result1 <- rbind(result, result_fake)

ggplot(result1, aes(condition, accuracy, group=condition, fill=condition)) +
  geom_boxplot(alpha=0.3) +
  facet_wrap(session~.) +
  theme_classic()

summary(lm(accuracy~condition,subset(result1, session==1)))
summary(lm(accuracy~condition,subset(result1, session==2)))
summary(lm(accuracy~condition,subset(result1, session==3)))
