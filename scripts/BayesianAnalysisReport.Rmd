---
title: "Report on The Social Route to Abstraction"
author: "Riccardo Fusaroli"
date: "14 May 2020"
output: html_document
---

The Markdown script contains the analyses for the study The Social Route to Abstraction. Notice that it is accompanied by a Supplementary Material document that provides additional motivation and detail for the statistical choices made here   
...
# Load data and define correct data types
...
H1. Groups will outperform individuals in the training sessions across the three levels of complexity. 
...
H3. Variability in group performance will be contingent on cognitive diversity, here operationalized as the semantic distance between group members' dialogical contributions. 
...
We use a mixed logistic regression model to assess main effects of condition (pair vs. individual) and complexity (session 1 to 3). We account for individual variability (random intercept plus random slopes due to session) and stimulus variability (random intercepts). Hypothesis testing is on the % scale

### Model 1

This model tests hypothesis 1. That is, it test training performance as a function of condition and complexity
...
# check the priors
H1_m_prior <- readRDS(here("models", "H1_m_prior.rds"))
pp_check(H1_m_prior, nsamples=100)
```
...
H1_m <- readRDS(here("models", "H1_m1.rds"))

pp_check(H1_m, nsamples=100)
...
p1 <- conditional_effects(H1_m)[[3]]

p1$condition <- factor(c("Individuals", "Groups"))

ggplot(p1, aes(session, estimate__, color = condition )) + 
  geom_line() +
  geom_point(size = 4) + 
  geom_errorbar(aes(ymin=estimate__-se__, ymax=estimate__+se__), width=.1) + 
...
Model 2 test hypothesis 2a, that groups will outperform individuals in the transfer task across levels of complexity. 
...
H2a_m_prior <- readRDS(here("models","H2a_m_prior.rds"))
pp_check(H2a_m_prior, nsamples=100)
```
...
```{r}
### Let's check model quality
H2a_m <- readRDS(here("models","H2a_m.rds"))
pp_check(H2a_m, nsamples=100)
...
p2 <- conditional_effects(H2a_m)[[3]]

p2$condition <- factor(c("Individuals", "Groups"))

ggplot(p2, aes(session, estimate__, color = condition )) + 
  geom_line(position=position_dodge(.2)) +
  geom_point(size = 4, position=position_dodge(.2)) + 
  geom_errorbar(aes(ymin=estimate__-se__, ymax=estimate__+se__), width=.1, position=position_dodge(.2)) + 
  scale_x_continuous(breaks=c(1, 2, 3)) +
  scale_color_brewer(palette="Dark2") +
...
This model test hypothesis H2b: Groups will outperform individuals in the first cycle of each session (before any stimulus tokens get repeated)   

Run the same model as for hypothesis 1 (model 1), but subset data to only include the first cycle (= 32 aliens).
...
H2b_m_prior <- brm(
  ...
  cores = 2,
  iter = 4000,
  file = here("models","H2b_m_prior"),
  control = list(adapt_delta = 0.9, max_treedepth = 20)
  )

H2b_m_prior <- readRDS(here("models","H2b_m_prior.rds"))
pp_check(H2b_m_prior, nsamples=100)
```
...
```{r}
### Let's check model quality
H2b_m <- readRDS(here("models", "H2b_m.rds"))
pp_check(H2b_m, nsamples=100)
...
p3 <- subset(d1, cycle==1)
p3$AccuracyP <- Preds[1,]
p3$Accuracy <- as.numeric(p3$correct)-1

p3$condition <- as.factor(p3$condition)
levels(p3$condition) <- c("Groups","Individuals")

p3$session <- as.factor(p3$session)
levels(p3$session) <- c("Low","Intermediate", "High")


ggplot(p3, aes(trial,Accuracy, color = condition)) +
...
ggsave(here("plots", "figure3.pdf"), Fig3, width = 5,height = 4)
ggsave(here("plots", "Figure3.svg"), Fig3, height = 10, width = 10)

```

## Model 4

Model 4 test hypothesis 3, that is, that variability in group performance will be contingent on the degree of cognitive diversity in the group. Here we operationalize cognitive diversity as the semantic distance between group members' dialogical contributions. 
The semantic variable is preprocessed in another Rmarkdown script, NLP_PreProcessing.Rmd, and imported together with the debriefing survey data.     
...
```{r}
mSSemLvl_prior <- brm(PerformanceTrainingSemanticLevelSession_f, 
                FinalData, chains=2, cores=2, 
                prior = prior,
                sample_prior = "only",
                file = here("models", "PerformanceTrainingSemanticLevelSession2_prior"),
                control=list(adapt_delta=0.99))

mSSemLvl_T_prior <- brm(PerformanceTestSemanticLevelSession_f, 
                FinalData, chains=2, cores=2, 
                prior = prior,
                sample_prior = "only",
                file = here("models", "PerformanceTestSemanticLevelSession2_prior"),
                control=list(adapt_delta=0.99))


mSSemLvl_prior <- readRDS(here("models", "PerformanceTrainingSemanticLevelSession2_prior.rds"))
pp_check(mSSemLvl_prior, nsamples=100)

mSSemLvl_T_prior <- readRDS(here("models", "PerformanceTestSemanticLevelSession2_prior.rds"))
pp_check(mSSemLvl_T_prior, nsamples=100)
```
...
mSSemLvl <- brm(PerformanceTrainingSemanticLevelSession_f, 
                ...
                prior = prior,
                sample_prior = T,
                file = here("models", "PerformanceTrainingSemanticLevelSession2"),
                control=list(adapt_delta=0.99))
...
mSSemLvl_T <- brm(PerformanceTestSemanticLevelSession_f, 
                ...
                prior = prior,
                sample_prior = T,
                file = here("models", "PerformanceTestSemanticLevelSession2"),
                control=list(adapt_delta=0.99))

mSSemLvl <- readRDS(here("models", "PerformanceTrainingSemanticLevelSession2.rds"))
pp_check(mSSemLvl, nsamples=100)

mSSemLvl_T <- readRDS(here("models", "PerformanceTestSemanticLevelSession2.rds"))
pp_check(mSSemLvl_T, nsamples=100)
```
...
FinalData <- FinalData %>% mutate(
  SemanticDiversity = ((SemanticLevelTraining - min(SemanticLevelTraining, na.rm=T)) / max(SemanticLevelTraining - min(SemanticLevelTraining, na.rm=T), na.rm=T))
)

# Set fixed/consistent colors 
palette1 = setNames(object = scales::hue_pal()(4), nm = c("1", "2", "3"))

FinalData$Session <- as.factor(FinalData$Session)

p1 <- ggplot(FinalData, aes(SemanticDiversity, PerformanceTraining)) +
  geom_point() + 
  ...
  labs(title = "Training", y = "Performance (accuracy)")

p2 <- ggplot(FinalData, aes(SemanticDiversity, PerformanceTest)) + 
  geom_point() + 
  geom_smooth(method=lm, color = "red") + 
  theme_classic() +
  #scale_color_brewer(palette="Dark2") +
...
p3 <- ggplot(FinalData, aes(SemanticDiversity, PerformanceTraining, color=Session, group=Session)) +
  geom_point() + 
  ...
  #scale_color_brewer(palette="Dark2") +
  scale_color_manual(values = palette1) +
  labs(x = "Semantic diversity (training)", y = "Performance (accuracy)", color = "Complexity")

p4 <- ggplot(FinalData, aes(SemanticDiversity, PerformanceTest, color=Session, group=Session)) + 
  geom_point() + 
  geom_smooth(method=lm) + 
  theme_classic() +
  #scale_color_brewer(palette="Dark2") +
  ...
  labs(x = "Semantic diversity (training)", color = "Complexity")

(p1 + p2) / (p3 + p4) + plot_annotation(tag_levels = 'A')

ggsave(here("plots", "figure5.pdf"), width = 8,height = 8)
ggsave(here("plots", "Figure5.svg"), width = 8, height = 8)
...
First we test for differences between the conditions. Next we test if the motivational factors influence/confound performance differences. 
...
```{r}
Motivation_m_prior <- brm(Motivation_f, 
                    family = cumulative, 
                    chains = 2,
                    cores = 1,
                    prior = prior,
...
pp_check(Motivation_m_prior, nsamples=100)
...
pp_check(Motivation_m, nsamples=100)
```
...
```{r}
Effort_m_prior <- brm(Effort ~ 1 + Condition + (1 | gr(Pair, by=Condition)), 
                family = cumulative, 
                    chains = 2,
                    cores = 1,
                    prior = prior,
...
pp_check(Effort_m_prior, nsamples=100)
...
pp_check(Effort_m, nsamples=100)
```
...
pp_check(RT_m_prior, nsamples=100)
...
## Test the relation between control variables and performance

# Does effort predict training performance? 
...
mTrainingSEffort <- brm(PerformanceTrainingEffortSession_f, 
                        FinalData, 
                        chains=2, 
                        cores=1, 
                        control=list(adapt_delta=0.99))
...
# Does motivation predict training performance? 
...
mTrainingSMotivation <- brm(PerformanceTrainingMotivationSession_f, 
                            FinalData, 
                            chains=2, 
                            cores=1, 
                            control=list(adapt_delta=0.99))
...
# Does effort predict transfer performance
