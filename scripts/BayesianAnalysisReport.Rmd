...
## NB. due to silly syntax, effects for each condition (dyad vs individuals) are estimated for each subject ID.
## Individuals are 1-27, Dyads are 30 to 56

TrainPerformance <- d1 %>% 
  group_by(subject, condition, session) %>% 
  summarize(TrainingPerformance = mean(Accuracy,na.rm=T))

TestPerformance <- d2 %>% 
  group_by(subject, condition, session) %>% 
  summarize(TestPerformance = mean(Accuracy,na.rm=T))

d3 <- merge(TrainPerformance, TestPerformance, all=T)

## Training Learning
TrainLearning <- d1 %>% 
  group_by(subject, session) %>% 
  summarize(TrainingPerformance = mean(as.numeric(correct)-1,na.rm=T))


ggplot(d3, aes(TrainingPerformance, TestPerformance)) + 
