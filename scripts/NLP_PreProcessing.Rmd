...
PerformanceTraining <- d %>% subset(test==0) %>%
  mutate(Preds = predict(Analysis1a_m)) %>%
...
PerformanceTest <- d %>% subset(test==1) %>%
  mutate(Preds = predict(Analysis1b_m)) %>%
  subset(condition == "Dyads") %>%
  group_by(subject, session) %>%
  dplyr::summarise(
    PerformanceP = mean(Preds, na.rm=T),
...
PerformanceCycle <- d %>% subset(cycle==1) %>%
  mutate(Preds = predict(Analysis1aTrial_m3)) %>%
...
write_csv(FinalData, here("data", "IndividualDifferences.csv"))
```
